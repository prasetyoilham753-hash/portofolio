...
const ctxMap = new WeakMap<HTMLElement, CtxRecord>();
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (e.touches.length > 0) {
        ...
        if (globalMouse) {
          targetMouse[0] = Math.max(0, Math.min(1, touch.clientX / window.innerWidth));
          targetMouse[1] = Math.max(0, Math.min(1, 1.0 - (touch.clientY / window.innerHeight)));
        } else {
          const rect = canvas.getBoundingClientRect();
